...
//         }
//         try {
//             const {data}=await axios.post('/api/blog/delete',{id:blog._id})//api
//             if(data.success){
...
//         try {
//             const {data}=await axios.post('/api/blog/toggle-publish',{id:blog._id})//api
//            if(data.success){
//                 toast.success(data.message)
//                 await fetchBlogs()
//             }else{
...
const BlogTableItem = ({ blog, fetchBlogs, index }) => {
  const { title, createdAt, comments = [] } = blog;  // 👈 include comments
  const BlogDate = new Date(createdAt);
  const { axios } = useAppContext();
  ...
  return (
    <>
      ...
      {/* 👇 Show comments below each blog row */}
      {comments.length > 0 && (
        <tr>
          <td colSpan="5" className="px-4 py-2 bg-gray-50">
            ...
            <ul className="list-disc pl-5 text-sm text-gray-700">
              {comments.map((c, i) => (
                <li key={c._id || i}>{c.text || c.content}</li>
              ))}
            </ul>
          </td>
        </tr>
